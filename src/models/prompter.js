...
const PROMPT_FILE_KEYS = new Set([
    'conversing',
    'coding',
    'saving_memory',
    'bot_responder',
...
function resolvePromptFilePath(filePath) {
    return path.isAbsolute(filePath) ? filePath : path.resolve(process.cwd(), filePath);
}

function loadPromptFiles(profile) {
    if (profile.prompt_files === undefined) {
        return;
    }
    if (profile.prompt_files === null || typeof profile.prompt_files !== 'object' || Array.isArray(profile.prompt_files)) {
        throw new Error('profile.prompt_files must be an object mapping prompt keys to file paths.');
    }
    for (const [key, filePath] of Object.entries(profile.prompt_files)) {
        if (!PROMPT_FILE_KEYS.has(key)) {
            throw new Error(`Unsupported prompt_files key "${key}". Supported keys: ${Array.from(PROMPT_FILE_KEYS).join(', ')}`);
        }
        if (typeof filePath !== 'string' || filePath.trim() === '') {
            throw new Error(`prompt_files.${key} must be a non-empty file path.`);
        }
        const resolvedPath = resolvePromptFilePath(filePath);
        try {
            profile[key] = readFileSync(resolvedPath, 'utf8');
        } catch (err) {
            throw new Error(`Failed to load prompt file for "${key}" from "${filePath}" (${resolvedPath}): ${err.message}`);
        }
    }
}
